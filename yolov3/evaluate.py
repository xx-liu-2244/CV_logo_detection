    ...
    def evaluate(self):
        ...
        with open(self.annotation_path, 'r') as annotation_file:
            for num, line in enumerate(annotation_file):
                ...
                image_path = annotation[0].split(',')[0]
                image_name = image_path.split('/')[-1]
                image = cv2.imread(image_path)                    
                bbox_data_gt = np.array([list(map(int, box.split(','))) for box in annotation[0].split(',')[1:]])
                l=[]
                for i in bbox_data_gt:
                       for t in i:
                            l.append(str(t))
                bbox_data_gt = l
                ...
                with open(ground_truth_path, 'w') as f:                   
                        class_name = classes_gt
                        xmin, ymin, xmax, ymax = bboxes_gt
                        #list(map(str, bboxes_gt))
                        bbox_mess = ' '.join([classes_gt, xmin, ymin, xmax, ymax]) + '\n'
                        f.write(bbox_mess)
